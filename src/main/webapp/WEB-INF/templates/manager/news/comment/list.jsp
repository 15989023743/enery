...
<!--
	function actionEvent(methodname,status){
		...
		if(validateIsSelect(form.all, form.stylesids)){
			form.action='product-style!'+methodname;
...
<form id="ListForm" name="ListForm" action="product-style" method="post" >
   ...
   <td height="18">
      ...
      <tr>
        ...
        <table style="width:100%" align="center" class="TableBlock1" >
		   ...
		   <tr>
		    <td nowrap class="TableContent" width="10%" align="center">样式名称</td>
		    <td nowrap class="TableData" width="38%" colspan="3"><input type="text" name="filter_LIKE_name" value="${param['filter_LIKE_name']}"  class="SmallInput" style="width:97%" maxlength="30" size="30"/></td> 
		   </tr>
		   <tr>
			<td align="center" class="TableContent" colspan="6">
				...
				<input type="button" class="SmallButton" name="b_act_new" onclick="input()" value="新增样式" style="width:60" />&nbsp;&nbsp;&nbsp;&nbsp;
				<input type="button" class="SmallButton" name="b_act_new" onclick="javascript:window.location.href='product'" value="返回" style="width:60" />
			</td>
		   </tr>		   		   
		</table>
			<!--过滤条件结束-->
        ...
        <table style="width:100%" align="center" class="TableBlock1" onmousemove="changeto()" onmouseout="changeback()">
          <tr align="center">
            ...
            <td width="3%" class="TableHeader1" colspan="1">操作</td>
            <td width="40%" class="TableHeader1"><a href="#" onclick="sort('name')">名称</a></td>
            <td width="5%" class="TableHeader1"><a href="#" onclick="sort('visible')">在售状态</a></td>
            <td width="10%" class="TableHeader1">产品图片</td>
          </tr>
          <!-- 			-->
          ...
          <tr>
			...
			<td class="TableData" align="center"><input type="checkbox" name="stylesids" value="${id}" /></td>
            <td class="TableData" align="center"><a href="#" onclick="input('${id}')">修改</a></td>
            <td class="TableData">${name}</a></td>
            <td class="TableData"><s:if test="visible">在售</s:if><s:else>停售</s:else></td>
            <td class="TableData"><div align="center"><img src="${ctx}${imageFullPath}" width="50"></div></td>
          </tr>
          </s:iterator>
        </table>
		<!--业务表格区域结束-->
            ...
            <td width="8%"><input type="checkbox" name="all" <c:if test="${fn:length(page.result)<1}"> disabled="disabled" </c:if>
             onclick="javascript:allselect(this, this.form.stylesids)"/>全选</td>
              ...
              <c:if test="${fn:length(page.result)<1}">disabled="disabled"</c:if>
               class="SmallButton"  onClick="javascript:actionEvent('visible',true)" value=" 上 架 "> &nbsp;&nbsp;
              <input name="disable" type="button" class="SmallButton" 
              <c:if test="${fn:length(page.result)<1}">disabled="disabled"</c:if>
              onClick="javascript:actionEvent('visible',false)" value=" 下 架 "></input>
            </td>
          </tr>
        </table>
      </td>
