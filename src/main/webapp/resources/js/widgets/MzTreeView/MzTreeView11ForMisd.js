...
//按照由页面传过来的KEY值对页面进行初始化 chekcBox的ID结构为  check_sid
...
//			document.getElementById("check_"+args[i]).checked=true;
//		}
//	}
}
//浏览器类型及版本检测
...
{	
  ...
  if(isRoot){HTML +="代码选择";}//隐藏根节点
	  ...
	  HTML += ("<input type='checkbox' id='check_"+sid+"' name='check' value='"+node.id+"' onclick=\""+ this.name +".checkOnclick('"+sid+"','"+id+"')\"/>")
	 /* 	  
	  	  "<IMG "+
	    "align='absMiddle' "+
...
//			var parentkeykey = this.node[sid].sourceIndex;
	...
	values = values.replace(/,}/g,"}");//正则表达
		...
		checkedValue["valueObj"]=rtv;
		checkedValue["codeKey"]=arguments["codeKey"];
		checkedValue["codeValue"]=arguments["codeValue"];
	window.returnValue = checkedValue;
    window.close();
};

/**
 * 为MISD返回格式化的多选值 驾车,驾船,驾机,    ,01,02,03
 * 2008-12-06   lf
 */ 
MzTreeView.prototype.getCheckedBoxForMisd = function(){
	var checkedValue=new Array();
	var obj = document.getElementsByName('check');
	var keys = "";//要返回的key字符串
	var values = "";//要返回的values字符串
	for(i=0;i<obj.length;i++){
		if(obj[i].checked){
			var sid = obj[i].id.substring(obj[i].id.indexOf("_")+1)
//			if(i == 0){
...
/**
 * FunName:clearAllOnClick
 * Description:给MISD字段返回空字符串(清空字典项)
 * 2008-12-06   lf
 */
...
MzTreeView.prototype.getAllChildrenCheckedBox = function(sid,id){
	var b = document.getElementById("check_"+sid).checked;
	...
	for(var i=0;i<len;i++){
		...
		document.getElementById("check_"+cldsid).checked = b;
		if(cldnodes[i].hasChild){
			this.getAllChildrenCheckedBox(cldsid,cldnodes[i].id);
		}
	}
};
//只有当父节点（不是根结点）的其他子结点为未选中时父节点的状态才改变
MzTreeView.prototype.getAllParentCheckedBox = function(sid,id){
	var thisnode = this.node[id];
	var obj = document.getElementById("check_"+sid);
	cldnodes = thisnode.parentNode.childNodes;
	for(var i=0;i<cldnodes.length;i++){
		var cldsid = cldnodes[i].sourceIndex.substring(cldnodes[i].sourceIndex.indexOf("_")+1);
		if(cldnodes[i].id != id && (document.getElementById("check_"+cldsid).checked == true)){
			return;
		}
	}
	if(thisnode.parentId >"1" ){
			document.getElementById("check_"+thisnode.parentNode.sourceIndex.substring(thisnode.parentNode.sourceIndex.indexOf("_")+1)).checked = obj.checked;
		this.getAllParentCheckedBox(thisnode.parentNode.sourceIndex.substring(thisnode.parentNode.sourceIndex.indexOf("_")+1),thisnode.parentId);
	}
};
//取消选择
...
MzTreeView.prototype.initGetPath=function(sourceId){
		Array.prototype.indexOf = function(item){
	    for(var i=0; i<this.length; i++)
	    {
	      if(this[i]==item) return i;
		...
		while(sourceId!="0" && sourceId!=""){
			var reg= new RegExp("(^|"+_d+")"+"[^"+_d+d +"]+"+ d+sourceId+"("+_d+"|$)", "g");
...
//在源代码里指定 MzTreeView 初始聚集到某个节点
//sourceId 节点在数据源里的 id
MzTreeView.prototype.initFocus = function(sourceId)
{
	Array.prototype.indexOf = function(item)
  {
    for(var i=0; i<this.length; i++)
  ...
  var d = this.divider;
  var path = this.initGetPath(sourceId);
  var root = this.node["0"], len = path.length;
  for(var i=1; i<len; i++)
  {
    if(root.hasChild)
...
MzTreeView.prototype.checkOnclick = function(sid,id){
	...
	if(thisnode.hasChild){
		this.getAllChildrenCheckedBox(sid,id);
	}//只有当父节点（不是根结点）的其他子结点为未选中时父节点的状态才改变
	if(thisnode.parentId >"1" ){
		this.getAllParentCheckedBox(sid,id);
	}
};
//返回对象
