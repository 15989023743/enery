...
function MzTreeView(Tname)
{
  ...
  this.toString = function()
  {
    ...
    return "<DIV class='MzTreeView' "+
      "onclick='"+ this.name +".clickHandle(event)' "+">"+ str +"</DIV>";
  };
...
//以CheckBoxinit中的值为基础进行树中的checkBox初始化.
MzTreeView.prototype.treeChecked=function(){
	for(var i=0;i<this.CheckBoxinit.length;i++){
		document.getElementById(this.CheckBoxinit[i]).checked=true;
	}
}
//浏览器类型及版本检测
...
MzTreeView.prototype.dataFormat = function()
{
  ...
  for (var id in this.nodes) {
  	a[a.length] = id;
  	var node_temp=this.nodes[id];
  	var data = this.getAttribute(node_temp, "data");
  	var a2=id.split("_");
  	if(a2[0]=="1"|| a2[0]=="0"){
  		a1[a1.length] = "1_"+id;
  	}else{
  		a1[a1.length] = data+"_"+id;
  	}
  }
  this.names = a.join(this._d + this._d);
  this.dataNames = a1.join(this._d + this._d);
  this.totalNode = a.length; a = null;a1=null;
};
...
{	
	if(node.hasChild){
		nodeCounts[nodeCounts.length] = node.id;
	}
 //根据node.sourceIndex(1_1)获取相应的信息(index:1,text:节点2..)
  ...
  if(isRoot){HTML +="代码选择";}//隐藏根节点
	 	...
	 	if(node.hasChild == false)
	  		HTML += ("<input type='checkbox' id="+data+"_"+node.sourceIndex+" name='zatzcheck' value='"+node.id+"' onclick=\""+ this.name +".checkOnclick('"+sid+"','"+id+"')\"/>")
	      else
	        HTML += ("<span style='display:none'><input type='checkbox' id="+data+"_"+node.sourceIndex+" name='zatzcheck' value='"+node.id+"' onclick=\""+ this.name +".checkOnclick('"+sid+"','"+id+"')\"/></span>")
	    //增加处理当节点是第一层次时,用红色显示
		if(node.sourceIndex.substring(0,node.sourceIndex.indexOf("_"))=="1") 
			  HTML+=
			 "<A "+
			...
			"onclick=\"return "+ this.name +".nodeClick('"+ id +"')\"><FONT COLOR=red>"+ node.text +//作为节点文字信息的单击事件
		  "</FONT></A></NOBR></DIV>";
	    else 
			  HTML+=
			 "<A "+
...
MzTreeView.prototype.getCheckedBox = function(){
	var checkobj = document.getElementsByName('zatzcheck');
	var keys = "";//要返回的key字符串
	...
	for(i=0;i<checkobj.length;i++){
		if(checkobj[i].checked){
			var sidArray=checkobj[i].id.split("_");
			var sid=sidArray[2];
			var parentKeys = sidArray[1];

			
			//如果父节点是最根结点则保存value
			var currNode = this.node[checkobj[i].value];
...
MzTreeView.prototype.cancleCheckedBox = function(){
	var obj = document.getElementsByName('zatzcheck');
...
MzTreeView.prototype.initCheckedBox = function(){
	...
	var arguments = window.dialogArguments;
    //var key="0001#20#32#0004#2000#2200#2206#0005#100#110"
      ...
      for (var k=0; k<root.childNodes.length; k++)
      {
        if (root.childNodes[k].sourceIndex == sourceIndex)
        {
    	  ...
    	  if(currSourceIndexArray[0]!="0"){
	          ...
	          if(this.CheckBoxinit==undefined){
	          	  //数组第一次存放数据时,无需进行"查重"校验
	          		this.CheckBoxinit[this.CheckBoxinit.length]=sourceIndex;
	          }else{
	          	  //数组有值时,需要进行"查重"操作
	          	  if(this.CheckBoxinit.indexOf(sourceIndex)==-1)
	          		this.CheckBoxinit[this.CheckBoxinit.length]=sourceIndex;
	          }
          }
          root = root.childNodes[k];
...
MzTreeView.prototype.closeAll = function(){
	for(var i = 1;i<=nodeCounts.length-1;i++){
//		setTimeout("tree.focusClientNode('"+nodeCounts[i]+"');tree.expand('"+nodeCounts[i]+"');", 3000);
		try{this.focusClientNode(nodeCounts[i])
		this.expand(nodeCounts[i]);}catch(e){}
	}
};
