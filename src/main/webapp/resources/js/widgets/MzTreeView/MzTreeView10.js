...
function MzTreeView(Tname)
{
  ...
  this.target   = "_self";
  this.name     = Tname;
  this.wordLine = false;//是否使用文字竖线模式
...
MzTreeView.prototype.nodeToHTML = function(node, AtEnd)
{
 ...
 if(isRoot){ 
  ...
  //////////////////////modified by zhangle//////////begin
  //将图片改为checkbox
//  HTML += "<input type='checkbox' id='check_"+node.id+"' name='check' onclick='if(this.checked == true){alert(\"字典key为:"+((node.hasChild) ?node.childNodes:"")+node.sourceIndex+"；值为:"+node.text+"\");}'/>"//////////////////////modified by zhangle//////////
  HTML += ""//"<input type='radio' id='radio_"+node.id+"' name='radio'/>"// onclick='"+this.name+".setSelectRadioBox();'/>"//////////////////////modified by zhangle//////////
  //////////////////////modified by zhangle//////////end
  	/*  
  	  "<IMG "+
    "align='absMiddle' "+
...
MzTreeView.prototype.expand   = function(id, sureExpand)
{
  ...
  if (area)
  {
    ...
    if(!node.isLoad)
    {
      ...
      if(node.hasChild && node.childNodes.length>500)
      {
...
MzTreeView.prototype.dblClickHandle = function(e)
{
  ...
  //返回信息并关闭窗口
  e = window.event || e; e = e.srcElement || e.target;
//  alert(e.tagName);alert(this.currentNode.text);return;
  if(e.tagName == "IMG" || this.currentNode.text == undefined){
  	alert("请双击文字信息进行选择！");
  	return;
  }
  var parentkeykey = this.currentNode.sourceIndex;//parentkey_key
  var keyValue = parentkeykey.substring(parentkeykey.indexOf("_")+1)+"#"+this.currentNode.text;//key#value字符串
  	  
  var records = window.dialogArguments;
  var result=new Array();
  result["value"]=parentkeykey.substring(parentkeykey.indexOf("_")+1);
  result["mcvalue"]=this.currentNode.text;
  result["clickField"]=records["clickField"];
  result["mcField"]=records["mcField"];
  window.returnValue=result;
  window.close();
  //window.returnValue = gxsnode;///////////////////////
//  var RFN = this.node["0"].childNodes[0].childNodes[0];
//  alert(RFN.text);///////////////////
 // alert(this.node[0].text);
  //window.close();
};
...
//////////////////////////////////////以下代码由zhangle添加/////////////////////////////////////////////
//判断checkbox的状态，如果父节点被选中则所有子结点也被选中
MzTreeView.prototype.setSelectCheckBox = function(){
	if(this.currentNode.hasChild){
		for(i=0;i<this.currentNode.childNodes.length;i++){
			if(document.getElementById('check_'+this.currentNode.id).checked){
				document.getElementById('check_'+this.currentNode.childNodes[i].id).checked=true;
			}
		}
	}
};
//获取单选框
MzTreeView.prototype.GetCheckedRadio = function(){
	...
	for(i;i<len;i++){
		if(obj[i].checked){
			...
			if(sid=="1"){
				alert("请不要选择根结点!");i=0;return;
				}
			//要返回的字符串
			var keyValue = parentkeykey.substring(parentkeykey.indexOf("_")+1)+"#"+this.node[sid].text;//key#value字符串
			return keyValue;
		}
	}
	if(i=len){
...
////////////////////////
function Gxsnode(key,value){
	//【property】
  this.key      = key;
  this.value   = value;
}

//var rootFirstNode = this.node[0].childNodes[0];
